registerPlugin({
    name: 'GioClock-v2.0',
    version: '2.0',
    description: 'Digital Clock with Date, Time, Developed by GioCombo.com',
    ...
    if (!config.channelIdNormal || !backend.getChannelByID(config.channelIdNormal)) {
        engine.log("El canal para la hora normal no existe. Deshabilitando el script...");
        return;
    }
    if (!config.channelId1 || !backend.getChannelByID(config.channelId1)) {
        engine.log("El canal para la Línea #1 no existe. Deshabilitando el script...");
        return;
    }
    if (!config.channelId2 || !backend.getChannelByID(config.channelId2)) {
        engine.log("El canal para la Línea #2 no existe. Deshabilitando el script...");
        return;
    }
    if (!config.channelId3 || !backend.getChannelByID(config.channelId3)) {
        engine.log("El canal para la Línea #3 no existe. Deshabilitando el script...");
        return;
    }
    if (!config.channelId4 || !backend.getChannelByID(config.channelId4)) {
        engine.log("El canal para la Línea #4 no existe. Deshabilitando el script...");
        return;
    }
    if (!config.channelIdDate || !backend.getChannelByID(config.channelIdDate)) {
        engine.log("El canal para la Fecha no existe. Deshabilitando el script...");
        return;
    }
    if (!config.channelIdDev || !backend.getChannelByID(config.channelIdDev)) {
        engine.log("El canal para Dev no existe. Deshabilitando el script...");
        return;
    }
